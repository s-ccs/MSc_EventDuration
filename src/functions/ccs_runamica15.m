...
function [varargout] = runamica15(dat,varargin)
...
bytesize = 4; % not used
basepath = [fileparts(which("pop_runamica.m")) filesep]; % directory where this .m file is located
...
pcakeep = -1;
   ...
   for i = 1:2:length(varargin) % for each Keyword
      ...
      elseif strcmp(Keyword,'tmpdir'), %added by ehinger 
         tmpdir = Value;
      else
...
if isnumeric(dat)
    ...
    disp(['Writing data file: ' fullfile(tmpdir,filename)]);
    fid = fopen(fullfile(tmpdir,filename), 'w');
    fwrite(fid, dat, 'float');
    fclose(fid)
    chans = size(dat,1);
    frames = size(dat,2);
    file = fullfile(tmpdir,filename);
end;             
...
if RUN_LOCALLY == 1
    ...
    fprintf("Welcome to B.Ehingers personal async AMICA. This starts the jobs in tmux in a new session 'amica'. Also deletes the temp-file \n")
    payload = [AMBIN ' ' outdir 'input.param && rm ' filename];
    system('tmux new-session -d -s amica'); % open new tmux session
    system(['tmux new-window -a -d -t amica -n ' filename ' "' payload '"']); % run the progress
    weights = [];
    sphere = [];
    mods = [];
