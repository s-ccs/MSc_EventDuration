...
# ╔═╡ 5f5da260-9afd-11ec-134e-d563e8a46068
begin
	using XDF
	using CairoMakie
	using DataFrames
...
# ╔═╡ 0c6e539d-6705-4f4b-b186-b9f0f74d1b82
md"# EventDuration - Marker Test"

# ╔═╡ 6a7aba41-5311-4d78-9988-afba39355533
md"### Import"

# ╔═╡ 2bc762a4-3c9e-4884-917a-31347cccf486
...
# ╔═╡ d19ac226-7f20-4ea2-8f5e-c4acf2ef1ed6
begin  # their own cell, so that I can reimport it without it rerunning all cells
	LSLimport = ingredients("../analysis/functions/LslTools_eventDuration.jl")
	LslTools = LSLimport.LslTools
end;

# ╔═╡ 4ad0e7d4-e79b-431d-bbeb-8f97af7d4bc1
begin 
	sub = 12
	run = 8
	streams = read_xdf(@sprintf("sub-%03i_ses-001_task-Default_run-%03i_eeg.xdf",sub,run))
end;

# ╔═╡ 2c222f3c-60e6-4c99-a90a-ce0beb7e7e72
begin
	streams_sync = deepcopy(streams)
	
	# Commented out dejitter, because can't use it on EEG data (bcs some EEG markers are missing)
	
	#streams_sync = LslTools.dejitter_ant!(streams_sync,"EEGstream","eegoSports-EE225_markersMarkers")
	#streams_sync = LslTools.dejitter!(streams_sync,"EEGstream")
	
	# move everything to EEG steady clock, e.g srate EEG 8001.3 => 8000Hz
	streams_sync = LslTools.sync_to_continuous!(streams_sync)
	
	eeg = LslTools.get_stream(streams_sync,"EEGstream")
	stim = LslTools.get_stream(streams_sync,"LSL_Markers_Matlab")
	eeg_stim = LslTools.get_stream(streams_sync,"eegoSports-EE225_markersMarkers")
end;

# ╔═╡ 21c97db3-a8f0-422c-9089-b7f12f4c8a80
md"### Inspect length of data"

# ╔═╡ 4593aff0-f461-44fc-93af-a06a03db26bb
begin
	println("Samples of EEG data: ", length(eeg["data"]))
	println("Number of EEG markers: ", length(eeg_stim["data"]))
	println("Number of Matlab markers: ", length(stim["data"]))
	println("Time of EEG recording: ", (length(eeg["data"])/8000)/60/2, " min")
		# /2 because on- & off-triggers are present
end

# ╔═╡ d1ce8d33-7715-47e9-b74e-f2c6ac4cb12f
md"### Check sample drops"

# ╔═╡ ce284760-d4ed-491e-b909-21216f82fbad
begin
	ixT = 1:1000
	scatter(collect(eeg["time"][ixT]),eeg["data"][ixT,end])
	current_figure()
end

# ╔═╡ 6340bf03-aedc-4e2a-a68b-bac903b0ab27
md"### Check drift"

# ╔═╡ 8de3ea16-1b9f-43ab-b5fd-70efa4581bda
begin
	# Plot EEG-Trigger minus LSL-Trigger
	scatter((eeg_stim["time"][1:length(stim["time"])] .- stim["time"][1:length(stim["time"])]).*1000)
	
	# Similar:
	# lines((evts.latency_eeg.-evts.latency_lsl)./8000*1000)
	
	current_figure()
end

# ╔═╡ fc914435-62aa-4484-a298-89441401efb1
md"### Create dataframe for events"

# ╔═╡ 79dbd16a-e7c1-46c6-832e-84a1d423c806
md"### Epoch"

# ╔═╡ 885ab68f-0b5f-4fc1-afda-f0989581df83
md"### Plot markers"

# ╔═╡ 9a52c73d-8392-4b54-b669-7949e8e64f93
md"""
Parallel Port Trigger? $(@bind latButtonEEG PlutoUI.CheckBox())
"""

# ╔═╡ 148343fc-3ca9-40c4-ac80-8b127b61adba
begin
evts = DataFrame(:onset=>stim["time"],:type=>stim["data"][:,1])
	evts.onset_eeg=eeg_stim["time"]#time_eeg_stim
	#effsrate = eeg["srate"]+/(8000/eeg["srate"])
	evts.latency_lsl = (evts.onset .- eeg["time"][1]) .* eeg["srate"] 
	evts.latency_eeg = (evts.onset_eeg .- eeg["time"][1]) .* eeg["srate"]

	if latButtonEEG
		evts.latency = evts.latency_eeg
	else
		evts.latency = evts.latency_lsl
	end

	#evts.latency = eeg_stim_samples .- eeg["data"][1,end]
	evts
end;

# ╔═╡ da85f326-5f69-4248-8466-bb100f3c62cf
data_e,times = Unfold.epoch(data = allowmissing(eeg["data"][:,end-1:end]'),
	tbl=evts,
	τ=(-0.01,0.02),
	sfreq=eeg["srate"]);

# ╔═╡ 4c7f8f60-33b9-4fc4-a131-6af6ceb788c5
let
	data_tmp = copy(data_e)
	data_tmp[ismissing.(data_tmp)] .= NaN
	data_tmp = disallowmissing(data_tmp)
	
	CairoMakie.image(times,1:2:size(data_tmp,3),data_tmp[1,:,1:2:end])
	vlines!(current_axis(),[0,0.0046, 0.0056])
	current_axis().title = latButtonEEG ? "Parallel Port" : "LSL"
	current_figure()
end

# ╔═╡ 3ccf6858-a7c6-401a-8fec-b45bc146e5c4
begin
	#ax = Axis(Figure())

	function plotMatrix(x::AbstractVector,y::Matrix)
	data_tmp = fill(NaN,size(y,1)+1,size(y,2))
	data_tmp[1:end-1,:] = y[:,:]
	time_tmp = repeat(vcat(x,NaN),size(y,2))
	endsize = size(data_tmp,1) * size(data_tmp,2)
	y = reshape(data_tmp,endsize)
	x = reshape(time_tmp,endsize)
	lines(x,y)
	end
	
	#plotMatrix(times[1:2:end],data_e[1,1:2:end,:])
	plotMatrix(times,data_e[1,:,251:2:end])
	current_axis().title = latButtonEEG ? "Parallel Port" : "LSL"
	current_figure()
	
end

# ╔═╡ 7409c9d4-6f9d-44d6-ba2e-d7f944b5ba21
md"### Inspect markers"

# ╔═╡ bf1c8b19-e909-4f61-90be-bab27508a06e
# Look at data
begin
	size(data_e)
	a = mean(data_e[1,:,251:2:end],dims=2)
	# Mean first third
	b = mean(a[1:Int(round(size(a)[1]/3))])
	# Max first third
	b1 = maximum(a[1:Int(round(size(a)[1]/3))])
	# Mean last third
	c = mean(a[Int(round(size(a)[1]*2/3)):end])
	# Min last third
	c1 = minimum(a[Int(round(size(a)[1]*2/3)):end])
	# Mean second third
	d = mean(a[Int(round(size(a)[1]/3)):Int(round(size(a)[1]*2/3))])
	# Min second third
	d1 = minimum(a[Int(round(size(a)[1]/3)):Int(round(size(a)[1]*2/3))])
	# Max second third
	d2 = maximum(a[Int(round(size(a)[1]/3)):Int(round(size(a)[1]*2/3))])
end;

# ╔═╡ 76ab396c-7381-4964-8bc0-b1c98a4a8ed9
begin
	threshold = b+0.1*(c1-b) # ~ 3*10^4
	println("Threshold: ",threshold)
	asdf = mean(data_e[1,:,251:2:end],dims=3);
	firstBiggerThanThresh = findfirst.(>(threshold),eachcol(asdf))
	earliestLSLTrigger = times[minimum(firstBiggerThanThresh)]
	latestLSLTrigger  = times[maximum(firstBiggerThanThresh)]
	println("Range of LSL triggers: ",round(earliestLSLTrigger*1000,digits=2)," - ",round(latestLSLTrigger*1000,digits=2)," ms")
	meanLSLTriggers = mean(times[firstBiggerThanThresh])
	println("Mean time of LSL triggers: ",round(meanLSLTriggers*1000,digits=2)," ms")
	medianLSLTriggers = median(times[firstBiggerThanThresh])
	println("Median time of LSL triggers: ",round(medianLSLTriggers*1000,digits=2)," ms\n")
	println("-> Subtract 5 ms from all LSL marker latencies for Oddball and Duration to\ncorrect shift in latency relative to EEG data!")
end
