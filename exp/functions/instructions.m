...
if cfg.do_P300
        ...
        while ~any(KbEventAvail(cfg.ix_responseDevice),[37])
            for i=1:2
                if i==1
                    ...
                    elseif cfg.gerInst
                        DrawFormattedText2(['<u>P300 Experiment<u>\n(10 Blöcke mit je 120 Versuchen)\n\n\nBeschreibung:\n\nWährend des Experiments ist eine Abfolge von Buchstaben (ABCDE) zu sehen.\n\nDie Aufgabe ist, auf den dargestellten Buchstaben durch Drücken des <color=0072BD>blauen<color=0> oder <color=EDB120>gelben<color=0> Knopfes\nzu reagieren, abhängig von der Zuordnung die am Anfang des Blocks gegeben wird.\n\n\nInstruktionen:\n\nZwischen Blöcken kannst du Pausen machen, wenn benötigt.\n\nDrücke den <color=0072BD>blauen<color=0> Knopf mit dem linken Zeigefinger und den <color=EDB120>gelben<color=0> Knopf mit dem rechten Zeigefinger.\n\nFixiere während des Experiments das Kreuz in der Mitte des Bildschirms.\n\nAntworte so genau und so schnell wie möglich.\n\n\n\n...\n> Drücke einen Knopf um das Experiment zu starten.'],'win',cfg.win,'sx','center','sy','center','xalign','center','yalign','center','xlayout','center');
                    end
                    WaitSecs(0.2);
                elseif i==2
                    % "Animation"
    ...
    elseif cfg.gerInst
        DrawFormattedText2(['Drücke den <color=0072BD>blauen<color=0> oder <color=EDB120>gelben<color=0> Knopf\nfür diese Buchstaben:'],'win',cfg.win,'sx','center','sy',1/5*cfg.rect(4),'xalign','center','yalign','center','xlayout','center');
        DrawFormattedText2(['> Drücke den <color=77AC30>grünen<color=0> Knopf um Block ',num2str(randomization_block.block(1)),' von ',num2str(cfg.P300.numBlocks),' zu starten.'],'win',cfg.win,'sx','center','sy',4/5*cfg.rect(4),'xalign','center','yalign','center','xlayout','center');
    end
...
if cfg.do_stimDur
    ...
    elseif cfg.gerInst
                DrawFormattedText2(['Drücke den <color=EDB120>gelben<color=0> Knopf, sobald der rote Punkt flickert.'],'win',cfg.win,'sx','center','sy',2/5*cfg.rect(4),'xalign','center','yalign','center','xlayout','center');
        DrawFormattedText2(['> Drücke den <color=77AC30>grünen<color=0> Knopf um Block ',num2str(randomization_block.block(1)),' von ',num2str(cfg.stimDur.numBlocks),' zu starten.'],'win',cfg.win,'sx','center','sy',4/5*cfg.rect(4),'xalign','center','yalign','center','xlayout','center');
    end
